...
func readFileAndSend(i int, q *Queue, connect redis.Conn) error {
...
func handleAppend(nums_arr []int, num int, q *Queue, connect redis.Conn) []int{
	new_nums_arr := append(nums_arr, num)
	if len(nums_arr) == len(new_nums_arr) {
